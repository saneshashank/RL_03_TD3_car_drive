...
MAX_CONSECUTIVE_CRASHES = 5 # 5
                               # Too high - will waste time on bad configs
                               # Hint: Usually 2-10

POLICY_NOISE = 0.2     # Noise for target policy smoothing
# NOISE_CLIP = 0.2        # Clip range for noise
POLICY_FREQ = 2         # Delayed policy updates (every 2 steps)
EXPL_NOISE = 0.2     # 0.05
...
class Actor(nn.Module):
    def __init__(self, state_dim, max_angle=15, max_speed=10):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, 2)  # 2 outputs: angle, speed
        )
    
    def forward(self, state):
        x = self.net(state)
        angle = torch.tanh(x[:, 0]) * self.max_angle  # Range: -15 to +15
        speed = torch.sigmoid(x[:, 1]) * self.max_speed + MIN_SPEED # Range: 2 to 10
        return torch.stack([angle, speed], dim=1)

class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        super(Critic, self).__init__()
        # Input: state (9 values) + action (2 values) = 11 total
        self.net = nn.Sequential(
...
class CarBrain:
    def __init__(self, map_image: QImage):
        ...
        self.state_dim = 9
        self.action_dim = 2  # angle and speed

        # Actor networks
        self.actor = Actor(self.state_dim, max_angle=TURN_SPEED, max_speed=SPEED)
        self.actor_target = Actor(self.state_dim, max_angle=TURN_SPEED, max_speed=SPEED)
        self.actor_target.load_state_dict(self.actor.state_dict())

        # Twin Critics
        self.critic1 = Critic(self.state_dim, self.action_dim)
        self.critic1_target = Critic(self.state_dim, self.action_dim)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2 = Critic(self.state_dim, self.action_dim)
        self.critic2_target = Critic(self.state_dim, self.action_dim)
        ...
        self.steps = 0
        # self.epsilon = 1.0 #0.01  # FIX ME! Exploration rate - starts too low! 
        ...
        self.expl_noise = EXPL_NOISE  # Start at 0.1
        self.min_expl_noise = 0.02    # Minimum noise
        self.noise_decay_steps = 10000
    ...
    def get_state(self):
        sensor_vals = []
        self.sensor_coords = []
        # 7 sensors spread across SENSOR_ANGLE
        half_angle = SENSOR_ANGLE / 2.0
        angles = np.linspace(-half_angle, half_angle, 7)
    ...
    def step(self, action):
        # action is [angle, speed] from Actor
        turn = action[0]  # Steering angle (-15 to +15)
        # reward = -0.1
        # reward -= abs(turn) * 0.5  # Add here
        # done = False

        speed = action[1]  # Speed (0 to 10)
        ...
        if car_center_val < 0.4:
            ...
            if self.prev_dist is not None:
                if dist < self.prev_dist:
                    reward += 10  # Reward for getting closer
                else:
    ...
    #     # if self.epsilon > 0.001: self.epsilon *= 0.9999
    #     return loss.item()
    ...
    def optimize(self):
        ...
        # --- UPDATE CRITICS ---
        with torch.no_grad():
            # Target policy smoothing (Trick 3)
            ...
            next_action = torch.clamp(next_action, 
                                    torch.FloatTensor([-TURN_SPEED, 0]), 
                                    torch.FloatTensor([TURN_SPEED, SPEED]))
            
            # Twin Q-values (Trick 1)
            target_q1 = self.critic1_target(ns, next_action)
            target_q2 = self.critic2_target(ns, next_action)
            target_q = torch.min(target_q1, target_q2)
            target_value = r + GAMMA * target_q * (1 - d)
        ...
        # --- DELAYED ACTOR UPDATE (Trick 2) ---
        self.steps += 1
        if self.steps % 2 == 0:  # Update every 2 steps
        ...
        if self.steps < self.noise_decay_steps:
            self.expl_noise = EXPL_NOISE * (1 - self.steps / self.noise_decay_steps) + self.min_expl_noise
        else:
            self.expl_noise = self.min_expl_noise
    ...
    def load(self):
        if os.path.isfile('last_brain.pth'):
            ...
            if 'start_pos' in checkpoint:
                x, y = checkpoint['start_pos']
                self.start_pos = QPointF(x, y)
            if 'targets' in checkpoint:
                self.targets = [QPointF(x, y) for x, y in checkpoint['targets']]
                if len(self.targets) > 0:
                    self.target_pos = self.targets[0]
                    self.current_target_idx = 0
...
class NeuralNavApp(QMainWindow):
    ...
    def save_model(self):
        try:
            self.brain.save()
            self.log("Model saved to last_brain.pth")
        except Exception as e:
            self.log(f"<font color='red'>Save Error: {str(e)}</font>")

    def load_model(self):
        try:
            self.brain.load()
            self.log("Model loaded from last_brain.pth")
            
            # Refresh Visuals
            self.full_reset() # Clear existing
            
            # Restore Car
            if self.brain.start_pos:
                self.setup_state = 1
                self.brain.car_pos = QPointF(self.brain.start_pos.x(), self.brain.start_pos.y())
                self.scene.addItem(self.car_item)
                self.car_item.setPos(self.brain.start_pos)
            
            # Restore Targets
            self.target_items = []
            for i, t_pos in enumerate(self.brain.targets):
                color = TARGET_COLORS[i % len(TARGET_COLORS)]
                is_active = (i == 0)
                ti = TargetItem(color, is_active, i+1)
                ti.setPos(t_pos)
            ...
            if len(self.brain.targets) > 0:
                self.setup_state = 2
                self.lbl_status.setText(f"LOADED. {len(self.brain.targets)} target(s). Press SPACE.")
            ...
            self.log(f"<font color='red'>Load Error: {str(e)}</font>")
    ...
    def game_loop(self):
        ...
        with torch.no_grad():
            ...
            action = self.brain.actor(state_tensor).squeeze(0).numpy()
            
            # Add exploration noise during training
            if training:
                noise = np.random.normal(0, self.brain.expl_noise, size=self.brain.action_dim)
                action = action + noise
                action = np.clip(action, [-TURN_SPEED, MIN_SPEED], [TURN_SPEED, SPEED])  # Clip to valid range
